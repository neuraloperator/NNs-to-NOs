...
from neuralop import get_model, LpLoss, Trainer
...
from utilities import LpLossAbs, MultiResTrainer, load_navier_stokes_pt
...
if __name__ == '__main__':
    ...
    save_name = model_name + '_ckpt_multi_res'
    ...
    for i in range(len(config.data.train_resolutions)):
        loader, _, _ = load_navier_stokes_pt(
            config.data.folder,
            train_resolution=config.data.train_resolutions[i],
            n_train=config.data.n_trains[i],
            batch_size=config.data.train_batch_sizes[i],
            test_resolutions=[config.data.train_resolutions[i]],
            n_tests=[1],
            test_batch_sizes=[1],
        ...
        train_loaders[str(config.data.train_resolutions[i])] = loader

    _, test_loaders, output_encoder = load_navier_stokes_pt(
        config.data.folder,
        train_resolution=config.data.train_resolutions[0],
        n_train=config.data.n_trains[0],
        batch_size=config.data.train_batch_sizes[0],
